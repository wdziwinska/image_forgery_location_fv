...
class CasiaPatchSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # combine RGB + FFT channels
        fft_patch = fft_patch[0:1]  # ensure single channel
        x = torch.cat([img_patch, fft_patch], dim=0)  # [4, ps, ps]
        y = (mask_patch > 0.5).float()  # [1, ps, ps]
        return x, y
...
class ConvNextUNet(nn.Module):
    def __init__(self):
        ...
        self.dec3 = nn.Sequential(nn.Conv2d(chs[2]*2, chs[2], 3, padding=1), nn.ReLU(),
                                   nn.Conv2d(chs[2], chs[2], 3, padding=1), nn.ReLU())
        self.up2 = nn.ConvTranspose2d(chs[2], chs[1], 2, stride=2)
        self.dec2 = nn.Sequential(nn.Conv2d(chs[1]*2, chs[1], 3, padding=1), nn.ReLU(),
                                   nn.Conv2d(chs[1], chs[1], 3, padding=1), nn.ReLU())
        self.up1 = nn.ConvTranspose2d(chs[1], chs[0], 2, stride=2)
        self.dec1 = nn.Sequential(nn.Conv2d(chs[0]*2, chs[0], 3, padding=1), nn.ReLU(),
...
def train_model():
    ...
    mlflow.set_experiment("convnext_patch_segmentation")
    with mlflow.start_run():
        mlflow.log_params({"patch_size":128, "patches_per_image":10, "epochs":num_epochs})
        for epoch in range(num_epochs):
            ...
            print(f"Epoch {epoch+1}/{num_epochs} tr={avg_tr:.4f} val={avg_v:.4f}")

        mlflow.pytorch.log_model(model, artifact_path="model")
        torch.save(model.state_dict(), "convnext_patch_segmentation.pth")
        mlflow.log_artifact("convnext_patch_segmentation.pth")
